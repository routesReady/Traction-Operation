...
app.get('/api/data/search', async (req, res) => {
  try {
    ...
    const enriched = rows.map(x => {
      ...
      return { ...buildRow(x, date, day, k), ...(savedRow || {}), sourceRow: x.sourceRow, sn: x.sourceRow, date, day, added: Boolean(savedRecord), editing: false, kavachFound: Boolean(k), kavachError: '' };
    });
...
function filterSaved(all, startDate, endDate, dir = '') {
  const from = startDate ? parseDMY(startDate) : null, to = endDate ? parseDMY(endDate) : null, targetDir = String(dir || '').trim().toUpperCase();
  return all.filter(x => {
    const d = parseDMY(x.searchDate); const rowDir = String(x.row?.dir || '').trim().toUpperCase();
    return d && (!from || d >= from) && (!to || d <= to) && (!targetDir || rowDir === targetDir);
  });
}
...
app.get('/api/data/saved', async (req, res) => {
  try {
    ...
    if (dir && !['UP', 'DN'].includes(String(dir).toUpperCase())) return res.status(400).json({ message: 'DIR must be UP or DN.' });
    const rows = filterSaved(await SavedRow.find({}).sort({ createdAt: -1 }).lean(), startDate, endDate, dir);
...
app.get('/api/summary', async (req, res) => {
  try {
    ...
    if (!['UP', 'DN'].includes(targetDir)) return res.status(400).json({ message: 'DIR must be UP or DN.' });
    ...
    res.json({ fromDate, toDate, dir: targetDir, totalRecords: filtered.length, rows });
  } catch (e) { res.status(500).json({ message: e.message }); }
});
